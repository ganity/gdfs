	...
	fn dispatcher(len: i32) -> (String, i32) {
		...
		// 		//let (data, tx) = (data.clone(), tx.clone());
				...
				let _ =  f.seek(SeekFrom::Start(0));
		 		...
		 		("fileid/".to_string() + &index.to_string() + "/" + &len.to_string() + "/" + &file_hash(hf).to_string(), index)
		 	//}
		 //});
	}
		...
		match stream.read(&mut fid){
			...
			Ok(n) => {
			  ...
			  //   let hf = HashFile { start: index, length: length as i32 };
		 		...
		 		// let av: Vec<&str> = v[3].split('.').collect();
		 		// let hfv = av[0].parse::<u64>().unwrap();
